...
const Login: React.FC<LoginProps> = ({ onLogin, onAdminLogin }) => {
  ...
  const handleForgotPassword = () => {
      alert("Neem contact op met de systeembeheerder om uw wachtwoord te resetten.\n\nE-mail: support@factuurmeester.nl");
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 relative">
      <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md z-10 border-t-4 transition-colors duration-300 border border-gray-300" 
        ...
        <div className="text-center mb-8">
          <h1 className={`text-3xl font-bold ${loginMode === 'admin' ? 'text-slate-800' : 'text-indigo-600'}`}>
            {loginMode === 'admin' ? 'FactuurMeester Admin' : 'FactuurMeester'}
          </h1>
